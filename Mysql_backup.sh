...
STORE_FOLDER="/backup/Captive/db/"
...
function do_backups() {
 ...
 # if [ "$backup_db" == "all" ]; then
  ...
  #  /usr/bin/mysqldump --all-databases | gzip -9 > $BACKUP_PATH/daily-$TODAY.sql.gz
    ...
    [[ ! -d "$BACKUP_PATH" ]] && mkdir -p "$BACKUP_PATH"
  	echo "   Creating $BACKUP_PATH/daily-$TODAY.sql.gz"
    /usr/bin/mysqldump --single-transaction  -h 10.1.32.85  -u bkpuser -pcaptivebkp@@ $db | gzip -9 > $BACKUP_PATH/daily-$TODAY.sql.gz
 # fi
...
find   /backup/Captive/db/captiveportal71/  -iname *.gz -mtime +10 -delete 
